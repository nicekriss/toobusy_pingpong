...
DEFAULT_MODEL_DOWNLOADS = {
    ...
    "aceStepAudioGen_v15XLTurbo.safetensors": {"repo": "Comfy-Org/ace_step_1.5_ComfyUI_files", "file": "split_files/diffusion_models/acestep_v1.5_xl_turbo_bf16.safetensors", "folder": "diffusion_models"},
    "aceStepAudioGen_tencQwen06bAce15.safetensors": {"repo": "Comfy-Org/ace_step_1.5_ComfyUI_files", "file": "split_files/text_encoders/qwen_0.6b_ace15.safetensors", "folder": "text_encoders"},
    "aceStepAudioGen_tencQwen4bAce15.safetensors": {"repo": "Comfy-Org/ace_step_1.5_ComfyUI_files", "file": "split_files/text_encoders/qwen_4b_ace15.safetensors", "folder": "text_encoders"},
    "aceStepAudioGen_vae.safetensors": {"repo": "Comfy-Org/ace_step_1.5_ComfyUI_files", "file": "split_files/vae/ace_1.5_vae.safetensors", "folder": "vae"},
...
function downloadText(st){if(!st)return'';if(st.state==='done')return'다운로드 완료';if(st.state==='error')return'실패: '+(st.err||'');if(st.state==='downloading')return'Downloading...';return'다운로드'}
...
function updateDownloadBadges(downloads){var active=false;Object.keys(downloads||{}).forEach(function(k){var st=downloads[k],txt=downloadText(st)||'download';if(st&&st.state==='downloading')active=true;document.querySelectorAll('[data-dl-target="'+cssAttr(k)+'"]').forEach(function(b){if(b.textContent!==txt)b.textContent=txt;b.title=(st&&st.state==='downloading')?('download '+(st.pct||0)+'%'):txt;b.disabled=!!(st&&st.state==='downloading')})});if(active){var u=document.getElementById('upinfo'),msg='model download running';if(u&&u.textContent!==msg)u.textContent=msg}}
...
function usesRoleBoard(m){return m==='klein'||m==='faceswap'}
...
function boardImageCard(a,i,order){ensureBoardPos(a,order);if(a.enabled===undefined)a.enabled=true;if(!usesRoleBoard(document.getElementById('mode').value)){return '<div class="bcard'+(a.enabled===false?' off':'')+'" style="left:'+a.board_x+'px;top:'+a.board_y+'px" ondragover="boardCardDragOver(event)" ondragleave="this.classList.remove(&quot;drop&quot;)" ondrop="boardCardDrop('+i+',event)"><div class="bdrag" onpointerdown="boardDragStart('+i+',event)"><span>INPUT IMAGE '+(order+1)+'</span><div class="btools"><button class="bdel" title="이미지 교체" onclick="openReplacePicker('+i+',event)">↺</button><button class="bdel" title="삭제" onclick="event.stopPropagation();event.preventDefault();boardDelete('+i+')">×</button></div></div><img src="'+a.data+'" title="'+esc(a.name||'')+'"><div class="bm"><div class="bn">'+esc(a.name||'image')+'</div><div class="brow"><label><input type="checkbox" '+(a.enabled!==false?'checked':'')+' onchange="boardSet('+i+',&quot;enabled&quot;,this.checked,false)">USE</label></div><textarea placeholder="note..." onchange="boardSet('+i+',&quot;note&quot;,this.value,false)">'+esc(a.note||'')+'</textarea></div></div>'}var t=typeForRoleDash(a.role,'image'),fm=faceMode(a),faceLike=(t==='character'||t==='face');var h='<div class="bcard'+(a.enabled===false?' off':'')+'" style="left:'+a.board_x+'px;top:'+a.board_y+'px" ondragover="boardCardDragOver(event)" ondragleave="this.classList.remove(&quot;drop&quot;)" ondrop="boardCardDrop('+i+',event)"><div class="bdrag" onpointerdown="boardDragStart('+i+',event)"><span>'+esc(a.role||'reference')+'</span><div class="btools"><button class="bdel" title="이미지 교체" onclick="openReplacePicker('+i+',event)">↺</button><button class="bdel" title="삭제" onclick="event.stopPropagation();event.preventDefault();boardDelete('+i+')">×</button></div></div><img src="'+a.data+'" title="'+esc(a.name||'')+'"><div class="bm"><div class="bn">'+esc(a.name||'image')+'</div><div class="brow"><label><input type="checkbox" '+(a.enabled!==false?'checked':'')+' onchange="boardSet('+i+',&quot;enabled&quot;,this.checked,false)">USE</label><select onchange="picked['+i+'].role=this.value;picked['+i+'].type=typeForRoleDash(this.value,&quot;image&quot;);renderAssets()">'+roleOptions(a.role||'ignore')+'</select></div>';
...
function renderReferenceBoard(){var photo=document.getElementById('rphoto'),face=document.getElementById('rface'),name=document.getElementById('rname'),goal=document.getElementById('rgoal'),acts=document.getElementById('racts'),m=document.getElementById('mode').value,c=CUSTOMS.find(function(x){return x.mode===m}),img=imageAt(0),faceImg=imageAt(1),text=document.getElementById('prompt').value.trim();
  ...
  if(needsReferenceBoard(m,c)){
    ...
    var roleBoard=usesRoleBoard(m);
    if(imgs.length||loras.length||audios.length){var order=0;photo.innerHTML=picked.map(function(a,i){if(!a)return'';if(a.kind==='image')return boardImageCard(a,i,order++);if(roleBoard&&a.kind==='audio')return boardAudioCard(a,i,order++);if(roleBoard&&(a.kind==='lora'||a.type==='lora'))return boardLoraCard(a,i,order++);return''}).join('')}else{photo.textContent=roleBoard?'이미지나 오디오를 이 보드에 드래그하거나 Add files를 눌러주세요':'이 워크플로우에 넣을 입력 이미지를 추가해주세요'}
    name.textContent=roleBoard?(imgs.length+' images · '+loras.length+' loras · '+audios.length+' audio'):(imgs.length+' input images');
    goal.textContent=text||(roleBoard?'카드를 움직이고 각 카드의 역할/모듈을 고르면 Reference Board 번들로 들어가요.':'역할명은 쓰지 않아요. 이미지가 워크플로우 입력 노드에 순서대로 들어갑니다.');
    acts.innerHTML=roleBoard?'<button onclick="openPicker(\'\')">Add files</button><button onclick="addLoraCard()">Add LoRA</button><input id="presetName" placeholder="Preset name"><button onclick="saveBoardPreset()">Save Preset</button><select id="presetSel"></select><button onclick="loadBoardPreset()">Load</button><button onclick="clearReference()">Clear cards</button>':'<button onclick="openPicker(\'image\')">이미지 추가</button><button onclick="clearReference()">Clear images</button>';if(roleBoard)fillPresetSelect();
    return
  }
  if(m==='faceswap'){
...
function renderAssets(){var box=document.getElementById('assets'),m=document.getElementById('mode').value,c=CUSTOMS.find(function(x){return x.mode===m});box.innerHTML='';box.className='assets'+(picked.length&&m!=='video'?' on':'');
  var boardMode=needsReferenceBoard(m,c),roleBoard=usesRoleBoard(m);
  picked.forEach(function(a,i){if(!a)return;if(a.enabled===undefined)a.enabled=true;var row=el('div','asset'+(a.enabled===false?' off':''));var board=(a.kind==='image'&&boardMode&&roleBoard),lora=((a.kind==='lora'||a.type==='lora')&&boardMode&&roleBoard);var role=board?roleSelect(a,i):'';var use=(board||lora||(a.kind==='image'&&boardMode))?'<label class="use"><input type="checkbox" '+(a.enabled!==false?'checked':'')+' onchange="picked['+i+'].enabled=this.checked;picked['+i+'].lora_enabled=this.checked;renderAssets()">USE</label>':'';var tools=board?boardTools(a,i):'';if(lora){tools=loraSelect('',a.lora_name||a.name||'','picked['+i+'].lora_name=this.value;picked['+i+'].name=this.value;renderAssets()')+'<input class="short" type="number" step="0.05" value="'+(a.lora_strength||1)+'" onchange="picked['+i+'].lora_strength=parseFloat(this.value||1);renderReferenceBoard()">'}row.innerHTML='<b>'+esc((lora?'LORA':a.kind).toUpperCase())+'</b>'+use+role+tools+'<span>'+esc(a.name||a.lora_name||'')+'</span><button>×</button>';row.querySelector('button').onclick=function(){picked.splice(i,1);renderAssets()};box.appendChild(row)});
  if(boardMode&&roleBoard){var add=el('div','asset');add.innerHTML='<b>LORA</b>'+loraSelect('loraAddSel','','')+'<button onclick="addLoraCard()">+ ADD</button>';box.appendChild(add);box.classList.add('on')}
  renderDirectorTimeline();
