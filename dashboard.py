...
function boardImageCard(a,i,order){ensureBoardPos(a,order);if(a.enabled===undefined)a.enabled=true;var t=typeForRoleDash(a.role,'image'),fm=faceMode(a),faceLike=(t==='character'||t==='face');var h='<div class="bcard'+(a.enabled===false?' off':'')+'" style="left:'+a.board_x+'px;top:'+a.board_y+'px"><div class="bdrag" onpointerdown="boardDragStart('+i+',event)"><span>'+esc(a.role||'reference')+'</span><button class="bdel" onclick="event.stopPropagation();boardDelete('+i+')">×</button></div><img src="'+a.data+'" title="'+esc(a.name||'')+'"><div class="bm"><div class="bn">'+esc(a.name||'image')+'</div><div class="brow"><label><input type="checkbox" '+(a.enabled!==false?'checked':'')+' onchange="boardSet('+i+',&quot;enabled&quot;,this.checked,false)">USE</label><select onchange="picked['+i+'].role=this.value;picked['+i+'].type=typeForRoleDash(this.value,&quot;image&quot;);renderAssets()">'+roleOptions(a.role||'ignore')+'</select></div>';
